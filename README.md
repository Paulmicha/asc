...
**NB** : Currently, no exit codes are used in any top-level entry points listed below. These fragments (and all those sourced in the "main shell") use `return` instead of `exit`. ASC attempts to follow [Google's Shell Style Guide](https://google.github.io/styleguide/shell.xml) where possible.
