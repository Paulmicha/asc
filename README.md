...
1. Copy/paste `sample.asc.yml` to `.asc.yml` & edit for easier *instance (re)init* implementation
...
The arguments to the script are indicated directly in `asc/instance/setup.sh` code comments - reproduced here for clarity ("docblock" extract) :
...
/path/to/my-project/    ← Project root dir ($PROJECT_DOCROOT)
  ...
  └── sample.asc.yml    ← Copy/paste to ".asc.yml" & edit for easier instance (re)init implementation
```
