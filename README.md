...
ASC is not a program; it's a generic, customizable "glue" between programs. Simple, loosely articulated bash script fragments.
...
ASC essentially relies on a relative global namepace. Its creation process involves building it "on the fly" in other side projects in which each step listed above (*Next steps*) is achieved by specific, custom scripts placed in a different `scripts` dir alongside `asc` in `PROJECT_DOCROOT`.
...
The file structure follows naming conventions. Typically facts, actions, subjects are used to categorize fragments of bash scripts meant to be sourced directly inside custom scripts (not included in the ASC project).
...
/path/to/project/           ← Project root dir ($PROJECT_DOCROOT).
  ...
  │   └── utilities/        ← ASC functions to hide complexity (see "Autoload").
...
**NB** : Currently, no exit codes are used in any top-level entry points listed below. These scripts (and all those sourced in the "main shell") use `return` instead of `exit`. ASC attempts to follow [Google's Shell Style Guide](https://google.github.io/styleguide/shell.xml) where possible.
