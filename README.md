...
ASC is not a program; it's a generic, customizable "glue" between programs. Simple, loosely articulated bash scripts with a minimalist ambition.
...
TL;DR ASC allows you to **maintain a common CLI** while easily swapping out [implementations](https://paulmicha.github.io/asc/about/tools-considerations.html) (i.e. "not marrying them").

ASC core provides a common set of commands to execute variable implementations of the following tasks :
...
ASC targets individual developers or relatively small teams attempting to streamline or implement a common workflow across older *and* newer projects (see *targeted audience* section below).

## HOW

ASC "core" (i.e. this repo - as opposed to ASC *extensions*) provides a minimal base for abstracting usual tasks while allowing to complement, combine, replace or add specific operations **with or without** [existing tools](https://paulmicha.github.io/asc/about/tools-considerations.html).
