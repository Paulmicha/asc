...
These steps are mere indications : in real life, you probably want to "wrap" these calls in your own scripts (e.g. to preset some arguments, etc), usually in the `$PROJECT_SCRIPTS` folder (`./scripts` by default). Examples and detailed explanations are provided in ASC source code comments.
...
Since every entry point sources `asc/bootstrap.sh` to load ASC functions and globals, these (`readonly`) variables are available everywhere. Their values are assigned during *instance init* which generates a single, git-ignored script : `$INSTANCE_LOCAL_FILES/global.vars.sh` (`scripts/asc/local/global.vars.sh` by default).
...
- `$INSTANCE_LOCAL_FILES/global.vars.sh` (`scripts/asc/local/global.vars.sh` by default), which is exporting the resulting read-only shell variables and get loaded on every command that "bootstraps" ASC (see `asc/bootstrap.sh`).
