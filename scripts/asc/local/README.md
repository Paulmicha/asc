...
- `default.mk` provides generic `make` "convenience" aliases corresponding to ASC primitives (actions by subject)

Extensions may also use this folder to store current instance-specific generated code.

See `u_instance_init()` in `asc/instance/instance.inc.sh` for more details.
