...
| **Scope** | Every `*.inc.sh` / `*.opt-inc.sh` that is sourced by a hardcoded `.` and is **not** derived by an ASC loader. New double-ext: `*.manual-inc.sh`. Rewrite each of those files’ **file-level** docblock to the templates below (same slice as the `git mv`). |
...
A third **filename** is not a third loader. Loaders never look at `*.manual-inc.sh`. The only way in is an explicit `.` in a caller that already exists.

`*.inc.sh` on a file that is **never** a name match currently **lies**: it looks autoloaded. `*.opt-inc.sh` under `asc/core/utils/` **lies** the other way: loaders never derive it (`utils/` is not a caller or hook dir). `manual-inc` is the self-explainable token: a human `.`s this.
...
Callers still `. asc/core/utils/fs.opt-inc.sh` (file gone). Wave 0 retargets those `.` lines. That is not optional bookkeeping.

---

## How to read the inventory

**Autoload** = a loader can derive this path from a dir name, a caller, or a hook. Keep `*.inc.sh` / `*.opt-inc.sh`. Extra explicit `.` of the same file (tests, `reinit` yaml fallback) is **not** a reason to rename.

**Manual** = no loader derives this path. Rename to `*.manual-inc.sh`, keep every existing `.`, rewrite the file-level docblock (see **File docblocks**).

**Dual** = kernel-hardcoded **and** eager name-match. Today that is only `asc/core/core.inc.sh` (on `ASC_INC` because `$subject` is `core`). Renaming it to `core.manual-inc.sh` **drops** the `ASC_INC` copy. That is intended: kernel already `.`s it before discovery. Test that `ASC_INC` no longer lists it.
...
Hardcoded `.` only. This is the whole work list.
...
| `asc/core/core.inc.sh` | `asc/core/core.manual-inc.sh` | `asc/bootstrap.sh` (and today also `ASC_INC` — drops) |
...
These were never `$subject/$subject.inc.sh`. `utils/` is not an active dir. The hub `.`s them on every heavy bootstrap.

### Explicit lazy cluster (never derived)

| Today | After | Real `.` callers (grep before moving) |
|-------|--------|--------------------------------------|
...
Shape: shebang, one-line description, how it is sourced, `@see` the real `.` path(s), convention line, then the first function docblock. No changelog `@see`. No inventory of helpers. No “slug lives in that other file” footnote.
...
The hub has no functions; keep the convention line anyway. Its body stays the four `.` of hub children.

### Hub children (sourced from `utils.manual-inc.sh`, which bootstrap `.`s)
...
Add the convention line where it is missing today (`shell.inc.sh`, and `core.inc.sh` in the kernel table). `@see` the **post-rename** hub path. If a slice still has `utils.inc.sh` on disk, `@see` that path until the hub rename lands, then retarget.
...
| `asc/yml/yml.inc.sh` | active dir `yml` (also explicit `.` in `reinit.sh` when `.env` is missing — keep that `.`, keep the suffix) |
...
`asc/core/core.inc.sh` is **not** in this table. It is kernel + name-match dual; it moves to `core.manual-inc.sh` above.

### Lazy `*.opt-inc.sh` (caller or hook derivation)

| File | Why autoload |
|------|----------------|
| `asc/test/test.opt-inc.sh` | `$subject.opt-inc.sh` for callers under `asc/test/` (`make test-core` → `asc/test/core.sh`). Extra explicit `.` from `generate.sh` and hooks **outside** `asc/test/` stays; **do not** rename |
| `asc/extensions/db/db/db.opt-inc.sh` | 2-level caller dir `db/` (`make db-dump`). Also explicit `.` from db hooks / `remote_instance` opt-inc |
...
| `asc/extensions/software/host/provision.opt-inc.sh` | `$action.opt-inc.sh` next to `provision.sh`; also `.` from `software.opt-inc.sh` |

If a later grep finds a `*.opt-inc.sh` whose **only** route is an explicit `.` and no caller/hook would derive it, it joins the manual table. Do not guess; grep.
...
| `*.manual-inc.sh` | A literal `. path` in a caller that already exists |
...
## Wave 0 — fs split callers (on disk, `.` lines stale)

`fs.opt-inc.sh` is gone. Source the file that defines the function.

| Caller | Needs |
|--------|--------|
| `asc/extensions/db/db/db.opt-inc.sh` | `. asc/core/utils/fs_compression.manual-inc.sh` (`f_fs_extract_in_place`, `f_fs_compress`) |
| `asc/extensions/db/db/dump_reduce.sh` | same |
| `scripts/asc/contrib/asc/drupalwt/new/project.sh` | `. asc/core/utils/fs_sync.manual-inc.sh` (`f_fs_merge_dirs`) |
...
Small `git mv` + retarget `.` + **file-level docblock** + tests. One cluster per slice.

1. **Kernel four** — bootstrap `.` lines; kernel header template; `test_asc_utils_inc_is_kernel_include` greps `utils.manual-inc.sh`; assert `ASC_INC` does **not** list `core.inc.sh` / `core.manual-inc.sh`.
...
3. **`global.opt-inc.sh` → `global.manual-inc.sh`** — every `.` site in the table above; caller-only header. Test: `test_f_global_aggregate_helpers_absent_from_kernel_bootstrap` still holds; `global.test.sh` sources the new path.
...
Replace the `fs.inc.sh` why-cell “hub `utils.inc.sh`” with hub `utils.manual-inc.sh`. Add one row: `(any)` / manual / `asc/core/utils/fs_compression.manual-inc.sh` / ❌ unless a caller `.`s it / not a name match, not caller-dir. Stop. Human continues.
...
- After Wave 1: `[[ " $ASC_INC " != *core.inc.sh* ]]` and `!= *core.manual-inc.sh*`.
...
- [ ] Wave 0: retarget stale `fs.opt-inc.sh` `.` lines; rewrite `fs_compression` / `fs_sync` file headers.
- [ ] Waves 1–4: `git mv` + `.` retarget + file-level docblock + the tests named above.
