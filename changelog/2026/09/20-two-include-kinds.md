...
| **Scope** | [asc-lightweight.mdc](../../../.cursor/rules/asc-lightweight.mdc) — two filename kinds only. Filename for the fs split stays `asc/core/utils/fs.opt-inc.sh`. |
...
`asc/core/utils/` is not a caller dir and has no `*.hook.sh`. Loaders never derive `fs.opt-inc.sh`. Callers `.` it. Same token as `db/db/db.opt-inc.sh`; the difference is the folder, not a new spelling.
