...
| **Shipped notes** | `@page` margins stay `.75cm .75cm`. One probe `page.pdf()` measures Chromium's page height and which headings already have a full section on that page (so `## 6` is not given extra `<br>`). Pagination then walks layout pages: orphan table fragment first, then orphan heading. Mermaid init font is 11px (~8pt body). Flowchart/gantt/sequence layout lengths (padding, wrappingWidth, nodeSpacing, …) scale from Mermaid’s 16px defaults by `11/16`. HTML labels inside `foreignObject` do not inherit the SVG `font-size`, so a measure-time stylesheet forces 11px/`--asc-mermaid-font-size` on `.nodeLabel` before `m.render()`; otherwise boxes are sized for the 16px browser default. Heading `id`s longer than 120 characters are shortened so Chromium PDF dest names stay within the 127-byte PDF name limit. Sequence-diagram messages that contain `;` / `--` / extra `:` are quoted as a Mermaid retry candidate. Probe heading matching prefers the PDF line whose Y matches the live layout, so a table of contents does not steal the keep-list. Failed Mermaid candidate renders leave `#dascMmd*` error SVGs in the document; those are removed so they do not print as a last-page “Syntax error in text”. Exploded `.code-line` spans no longer keep a `\n` between block lines (that doubled every tree/code line). |
