...
A bare name and `asc:` for the same hook name are one file. Either order (`pre-commit asc:pre-commit` or the reverse) sets `git_hook_context=asc`. Before comparing directories, the writer resolves each one with `realpath` (relative segments, `..`, and symlinks). `APP_DOCROOT=site` and `SITE_DOCROOT` set to the absolute path of that same directory are one directory. Two contexts that resolve to one directory, including a subject whose `_DOCROOT` resolves to `$PROJECT_DOCROOT`, `exit 1` before any write. Different contexts that resolve to different directories are different files.
...
The writer drops `APP_DOCROOT` as a destination. A hook name is written only when creating the file and the following `chmod` both return 0. The current writer reports completion without checking either. Cleanup for that name runs only after both succeed. It then deletes `$APP_DOCROOT/.git/hooks/<name>` when that path is set and its resolved path is not a destination just written for that name. Comparison uses the resolved paths, so a relative `APP_DOCROOT` that is the directory just written is kept. A failed create or `chmod` leaves the legacy file in place and `exit 1`s. The marker `automatically generated during "instance init"` shows that ASC wrote the file. It does not show that the file is still unmodified. This pass deletes a marked file at a non-destination even when someone has edited the body. An instance that wants to keep that edited file removes the marker sentence before reinit. A file without the marker stays. An old generated hook whose directory is no longer `APP_DOCROOT` is not found by this pass. The instance removes that file itself before reinit.
...
Cons: an instance that relies on `APP_GIT_INIT_CLONE` today gets no clone until it adds the hook implementation. A bare name moves from `APP_DOCROOT` to the instance repo. The writer then deletes a marked hook at the old path, including one that was edited in place, unless that path resolves to a destination just written or the marker sentence was removed. `site:pre-commit` `exit 1`s when `SITE_DOCROOT/.git/hooks` is missing. A subject that is not discovered yet is skipped by the clone call.
...
3. In `f_git_write_hooks`, parse each token as a bare hook name or `context:name`. Drop the `APP_DOCROOT` write destination. Argument 2 applies only to bare names. A context token with argument 2 set `exit 2`s. Resolve each hooks directory with `realpath` before collision checks and before cleanup. Write bare names and `asc:` into `$PROJECT_DOCROOT/.git/hooks` unless argument 2 set a directory for the bare names. Write any other context into `$SUBJECT_DOCROOT/.git/hooks`. Reject a context that is not `^[A-Za-z_][A-Za-z0-9_]*$`, and reject two contexts that resolve to one directory. Set `git_hook_context` in the generated script. Keep `cd "$PROJECT_DOCROOT"` and `hook -s 'git'`. `exit 1` when a listed context has no hooks directory. A name is eligible for cleanup only after its file was created and `chmod` returned 0. Then delete a marked hook at `$APP_DOCROOT/.git/hooks/<name>` whose resolved path is not one of those destinations. A marked file that was edited in place is still deleted. Removing the marker sentence keeps it.
...
7. Extend `asc/test/core/git_readme_toc.test.sh`. Keep `f_git_write_hooks 'pre-commit' "$dir"` writing that directory with `git_hook_context` empty. Assert, in a subshell, with two git repositories:
   ...
   - `site:pre-commit` and `app:pre-commit` whose directories are the same after `realpath`, including a relative path and a symlink to it, exit 1 and write nothing
   ...
   - `APP_DOCROOT=site` and `SITE_DOCROOT` set to that directory's absolute path write one hook and leave it in place
   - a destination that cannot be written leaves the marked legacy file in place
   - a marked legacy file that is not a resolved destination is removed, including one whose body was edited
   - a legacy file without the marker sentence stays
