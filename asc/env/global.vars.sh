...
# features provided by ASC core by default (overridable).
# @see asc/git/init.hook.sh
global APP_GIT_ORIGIN "[help]='Optional. Ex: git@my-git-origin.org:my-git-account/asc.git. Allows projects to have their own separate repo. If set, then by default \"instance init\" will clone that repo (once) and a default selection of Git hooks will be overwritten to trigger ASC hooks.'"
...
global PROJECT_SCRIPTS "[default]=scripts [help]='Path to custom scripts ~ commonly automated processes. ASC will also use this path to look for overrides and complements (alteration mecanisms).'"
