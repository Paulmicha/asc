...
The variables written and loaded using the scripts in this directory are shared across the rest of ASC scripts, which depend on them.

Some are common to any type of stack, while others are specific to certain variants.

## Conventions

To support variants using the same scripts, ASC relies on conventions.

The (over)writing happens during **stack init** phase. It copies files from the `asc/env/dist` dir into `asc/env/current`, while replacing their placeholders with the values generated by the stack init script (`asc/stack/init.sh`).
...
Given the following value for the '-s' or '--stack' arg: "drupal-7", and for the '-p' or '--provision' arg: "scripts", this script will attempt to copy the contents of all the following files (if they exist), append it to the file storing settings of the current local instance, `asc/env/current/.app.env.sh` :

- `asc/env/dist/drupal/.app.env.sh.dist`
- `asc/env/dist/drupal/.scripts_provision.env.sh.dist`
- `asc/env/dist/drupal/7/.app.env.sh.dist`
- `asc/env/dist/drupal/7/.scripts_provision.env.sh.dist`
...
The naming convention for variables "assembled" in `asc/stack/init.sh` is the prefix "P_" + the actual env var name, as they will be loaded by `asc/env/load.sh`.
