...
u_instance_init() {
  ...
  local p_ascii_app_docroot=''
  local p_ascii_app_git_origin=''
  local p_ascii_server_docroot=''
  local p_ascii_instance_type=''
  local p_ascii_instance_domain=''
  local p_ascii_dc_ns=''
  ...
  yaml_parsed_sp_init=''
  yaml_parsed_globals=''
  if [[ -f "asc.yml" ]] || [[ -f ".asc-local.yml" ]]; then
    if [[ -f "asc.yml" ]]; then
      u_instance_yaml_config_parse "asc.yml"
    fi
    if [[ -f ".asc-local.yml" ]]; then
      u_instance_yaml_config_parse ".asc-local.yml"
    fi
    if [[ -n "$yaml_parsed_sp_init" ]]; then
      ...
      if [[ -n "$YAML_INSTANCE_DOMAIN" ]]; then
        p_ascii_instance_domain="$YAML_INSTANCE_DOMAIN"
      fi
      if [[ -n "$YAML_DC_NS" ]]; then
        p_ascii_dc_ns="$YAML_DC_NS"
      fi
    fi
  fi
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -a) p_ascii_app_docroot="$2"; shift 2;;
      -s) p_ascii_server_docroot="$2"; shift 2;;
      -g) p_ascii_app_git_origin="$2"; shift 2;;
      -t) p_ascii_instance_type="$2"; shift 2;;
      -d) p_ascii_instance_domain="$2"; shift 2;;
      -c) p_ascii_dc_ns="$2"; shift 2;;
  ...
  if [[ -f "scripts/asc/override/.asc-local.$HOST_TYPE.yml" ]]; then
    if [[ -f ".asc-local.$HOST_TYPE.yml" ]]; then
      rm -f ".asc-local.$HOST_TYPE.yml"
    fi
    cp "scripts/asc/override/.asc-local.$HOST_TYPE.yml" ".asc-local.$HOST_TYPE.yml"
  fi
  if [[ -f "scripts/asc/override/.asc-local.$INSTANCE_TYPE.yml" ]]; then
    if [[ -f ".asc-local.$INSTANCE_TYPE.yml" ]]; then
      rm -f ".asc-local.$INSTANCE_TYPE.yml"
    fi
    cp "scripts/asc/override/.asc-local.$INSTANCE_TYPE.yml" ".asc-local.$INSTANCE_TYPE.yml"
  fi
  if [[ -f "scripts/asc/override/.asc-local.$HOST_TYPE.$INSTANCE_TYPE.yml" ]]; then
    if [[ -f ".asc-local.$HOST_TYPE.$INSTANCE_TYPE.yml" ]]; then
      rm -f ".asc-local.$HOST_TYPE.$INSTANCE_TYPE.yml"
    fi
    cp "scripts/asc/override/.asc-local.$HOST_TYPE.$INSTANCE_TYPE.yml" ".asc-local.$HOST_TYPE.$INSTANCE_TYPE.yml"
  fi
  yaml_parsed_sp_init=''
  yaml_parsed_globals=''
...
#
# To verify which files can be used (besides ".asc-local.yml" in this project
...
u_instance_yaml_config_load() {
  ...
  instance_yaml_config_root_files=".asc-local.yml
...
# @see u_instance_write_mk() in asc/instance/instance.inc.sh
#
