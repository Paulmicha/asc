...
u_stack_deps_get_lookup_paths() {
  ...
  u_stack_deps_add_lookup_variants_by_path "asc/provision"

  # App-related dependencies.
  u_stack_deps_add_lookup_variants_by_path "asc/app/$APP"
  ...
  for stack_preset in "${STACK_PRESETS[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      ...
      for sp_type in $sp_types; do
        u_stack_deps_add_lookup_variants_by_path "asc/$sp_type/presets/${stack_preset}"
      done
    fi
  done
}
