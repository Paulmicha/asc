...
global FILE_REGISTRY_HOST_LEVEL_PATH "[default]=$HOME/.local/state/asc/registry [help]='Specifies where the files used as key/value store backend should be written. Important note : when hosting multiple ASC projects and/or project instances on the same host, if this value differs, the host-level values won’t be shared (which defeats their purpose).'"
