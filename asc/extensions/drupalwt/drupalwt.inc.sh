...
# @exports ASC_DB_ID
# @exports DB_*
# @exports <ASC_DB_ID>_DB_*
#
# Important note : for multi-site setups, DWT uses the site ID as ASC_DB_ID.
# This is subject to collisions for cases where additional DB are manually set up.
#
# @example
#   # Default database :
#   u_dwt_db_set
#
#   # Multi-site - target a specific site's database :
#   u_dwt_db_set 'my_site_id'
#
u_dwt_db_set() {
  local p_site="$1"
  local v
  local db_var
  local dwt_sites_db_var
  local dwt_sites_db_var_isset
...
EOF
  ...
  for var_name in "${asc_globals_var_names[@]}"; do
    if grep -Fq "${token_prefix}${var_name}${token_suffix}" "$drupal_settings"; then
      ...
      # @see asc/extensions/drupalwt/app/global.docker-compose.vars.sh
  ...
  case "$DWT_MULTISITE" in true)
    local multisite_key
    local multisite_var
    local data_keys='domain dir install_profile config_sync_dir'

    for multisite_key in $data_keys; do
...
#   data_keys='domain dir install_profile config_sync_dir'
#   u_dwt_sites
#   for site_id in "${dwt_sites_ids[@]}"; do
