...
u_db_set_all() {
  ...
  if [[ -n "$ASC_DB_IDS" ]]; then
    u_array_add_once "$ASC_DB_IDS" db_ids
  fi
...
u_db_backup() {
  if [[ -z "$ASC_DB_DUMPS_BASE_PATH" ]]; then
    echo >&2
    echo "Error in u_db_backup() - $BASH_SOURCE line $LINENO: the required global 'ASC_DB_DUMPS_BASE_PATH' is undefined." >&2
...
u_db_restore_last() {
  ...
  u_db_restore "$(u_fs_get_most_recent $ASC_DB_DUMPS_BASE_PATH)" "$1" "$2"
}
...
u_db_routine_backup() {
  ...
  u_db_set $@
  db_routine_new_backup_file="$ASC_DB_DUMPS_BASE_PATH/local/$DB_ID/$(date +"%Y/%m/%d/%H-%M-%S")_$db_backup_file_middle.$db_backup_file_ext"
