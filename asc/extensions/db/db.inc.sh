...
##
# Gets DB credentials (opt-out available when ASC_DB_MODE = 'none').
#
...
u_db_get_credentials() {
  if [[ -z "$ASC_DB_DUMPS_BASE_PATH" ]]; then
    echo >&2
    echo "Error in u_db_get_credentials() - $BASH_SOURCE line $LINENO: the required global 'ASC_DB_DUMPS_BASE_PATH' is undefined." >&2
...
#
u_db_restore_last() {
  u_db_restore "$(u_fs_get_most_recent $ASC_DB_DUMPS_BASE_PATH)" "$@"
}
...
u_db_routine_backup() {
  ...
  u_db_get_credentials
  db_routine_new_backup_file="$ASC_DB_DUMPS_BASE_PATH/local/$DB_ID/$(date +"%Y/%m/%d/%H-%M-%S").dump"
  ...
  # global ASC_DB_BAK_ROUTINE_PURGE "[default]='1m:5,3m:3,6m:2,1y:1' [help]='Custom syntax specifying how many dump files to keep by age. Comma-separated list of quotas - ex: 1m:5 = for backups older than 1 month, keep max 5 files in that month.'"
  ...
  if [[ -n "$p_option" ]]; then
    case "$p_option" in
      ...
      initial)
        ...
        u_fs_file_list "$ASC_DB_DUMPS_BASE_PATH" 'initial.*'
        for initial_dump_match in $file_list; do
          dump_to_return="$ASC_DB_DUMPS_BASE_PATH/$initial_dump_match"
        done
        ;;
    esac
  else
    dump_to_return="$(u_fs_get_most_recent $ASC_DB_DUMPS_BASE_PATH)"
  fi
