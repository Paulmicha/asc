...
#   order to "bootstrap" ASC-based scripts remotely to operate that instance.
#
...
#
# @example
#   u_remote_exec_wrapper my_short_id asc/test/asc/global.test.sh
...
EOF
    ...
    parsed_yaml_remotes="$(u_yaml_parse "$hook_most_specific_dry_run_match" 'ascri_')"
  ...
  if [[ -f 'scripts/asc/local/remote-instances.sh' ]]; then
    ...
    u_yaml_get_keys "$parsed_yaml_remotes" 'ascri_'
    ...
    for remote_id in "${yaml_keys[@]}"; do
      ...
      if [[ -n "$ssh_use_agent_filter" ]]; then
        ...
        if [[ -z "$ssh_pubkey" ]] && [[ -z "$ASC_SSH_PUBKEY" ]]; then
          echo >&2
          echo "Error in u_remote_instances_setup() - $BASH_SOURCE line $LINENO: missing ASC_SSH_PUBKEY env var." >&2
          echo "-> Aborting (1)." >&2
          echo >&2
          exit 1
        else
...
#     # "afssh -f $(u_remote_get_pubkey_hex_md5_fingerprint $ASC_SSH_PUBKEY) -- -T remote_user@remote.instance.example.com"
#
u_remote_instance_add() {
  local p_id="$1"
  local p_host="$2"
  ...
  if [[ ! -d 'scripts/asc/local/remote-instances' ]]; then
    ...
    if [[ $? -ne 0 ]]; then
      echo >&2
      echo "Error in u_remote_instance_add() - $BASH_SOURCE line $LINENO: failed to create missing required dir scripts/asc/local/remote-instances." >&2
      echo "-> Aborting (1)." >&2
      echo >&2
      return 1
    fi
