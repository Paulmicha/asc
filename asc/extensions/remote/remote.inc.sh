...
u_remote_instance_add() {
  ...
  if [[ ! -d 'asc/env/current/remote-instances' ]]; then
    echo >&2
    echo "Error in u_remote_instance_add() - $BASH_SOURCE line $LINENO: dir asc/env/current/remote-instances is missing." >&2
    echo "-> Aborting (1)." >&2
    echo >&2
    return 1
  fi

  local conf="asc/env/current/remote-instances/${p_id}.sh"
...
#   asc/extensions/remote/remote/instances.
#
