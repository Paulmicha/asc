...
u_remote_sync_db_to() {
  ...
  if [[ -n "$p_option" ]]; then
    ...
    rst_dump_file="$(u_fs_get_most_recent $ASC_DB_DUMPS_BASE_PATH)"
  fi
  ...
  rst_dump_local_base_path="$ASC_DB_DUMPS_BASE_PATH/local/$DB_ID"
  rst_dump_remote_base_path="$ASC_DB_DUMPS_BASE_PATH/$p_id/$DB_ID"
...
u_remote_sync_db_from() {
  ...
  if [[ -n "$p_option" ]]; then
    ...
    case "$p_option" in new)
      rsf_remote_dump_file="$(asc/extensions/remote/remote/exec.sh "$p_id" "asc/extensions/db/db/get_dump.sh new")"
      rsf_remote_dump_file="${rsf_remote_dump_file#$REMOTE_INSTANCE_PROJECT_DOCROOT/}"
    esac
  else
    rsf_remote_dump_file="$(asc/extensions/remote/remote/exec.sh "$p_id" "asc/extensions/db/db/get_dump.sh")"
    rsf_remote_dump_file="${rsf_remote_dump_file#$REMOTE_INSTANCE_PROJECT_DOCROOT/}"
  fi
  ...
  rsf_dump_local_base_path="$ASC_DB_DUMPS_BASE_PATH/$p_id/$DB_ID"
  rsf_dump_remote_base_path="$ASC_DB_DUMPS_BASE_PATH/local/$DB_ID"
