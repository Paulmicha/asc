...
f_nested_asc_print() {
  ...
  if [[ -z "$p_docroot" || ! -d "$p_docroot" ]]; then
    echo "Error in f_nested_asc_print() - $BASH_SOURCE line $LINENO: invalid docroot '$p_docroot'." >&2
    return 1
  fi

  if [[ ! -f "$p_docroot/asc/bootstrap.sh" ]]; then
    echo "Error in f_nested_asc_print() - $BASH_SOURCE line $LINENO: not a ASC instance ('$p_docroot/asc/bootstrap.sh' missing)." >&2
    return 2
  fi
