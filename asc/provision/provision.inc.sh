...
#
# @see asc/provision/dependencies.sh
#
...
#
# TODO autoload includes using *dist.yml suffix (double extension pattern).
#
...
# @see u_provisioning_includes_get_lookup_paths()
#
...
##
# Gets provisioning includes lookup paths.
#
# @requires the following globals in calling scope :
# - $PROVISION_USING
# - $HOST_TYPE
...
# - $INSTANCE_TYPE
#
# @exports result in global $PROV_INCLUDES_LOOKUP_PATHS.
#
# @see u_provisioning_preprocess()
# @see asc/stack/setup.sh
#
u_provisioning_includes_get_lookup_paths() {
  export PROV_INCLUDES_LOOKUP_PATHS
  PROV_INCLUDES_LOOKUP_PATHS=()

  u_autoload_add_lookup_level "provision/" 'dist.yml' "$PROVISION_USING" PROV_INCLUDES_LOOKUP_PATHS "$HOST_OS"
  u_autoload_add_lookup_level "provision/" "${HOST_TYPE}_host.dist.yml" "$PROVISION_USING" PROV_INCLUDES_LOOKUP_PATHS "$HOST_OS"
...
u_provisioning_get_script() {
  ...
  u_autoload_add_lookup_level "asc/$p_subject/" "${p_operation}.sh" "$PROVISION_USING" script_lookup_paths '' '/'
