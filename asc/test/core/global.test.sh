...
# `asc/asc/global.opt-inc.sh`. There is no kernel `global.inc.sh`.
...
printf " %s" "$(type -t f_global_assign_value)"
printf " %s" "$(type -t global)"')"
  assertEquals 'init-only global helpers unset after kernel bootstrap' \
    '    ' "$out"
...
printf " %s" "$(type -t f_global_assign_value)"
printf " %s" "$(type -t global)"')"
  assertEquals 'init-only global helpers exist after opt-inc' \
    'function function function function function' "$out"
}
