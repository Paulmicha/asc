...
if f_thread_pileup_should_skip "$p_entry"; then
...
f_thread_supervised_run() {
  ...
  if ! f_thread_lock_acquire "$ASC_THREAD_ENTRY" "$p_lock_mode"; then
  ...
  delay_s="$(f_thread_delay_seconds "$p_retry_delay")"
...
f_thread_chown_human "data/threads/${p_entry}.yml"
f_thread_chown_human "data/threads/${p_entry}.lock"
