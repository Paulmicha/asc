...
u_asc_extend() {
  ...
  # strings on every call to u_asc_extend().
  ...
  for subject in $subjects_list; do
    ...
    for action in $actions_list; do
      ...
      primitive_values=''
      u_asc_primitive_values 'prefixes' "$p_path/$subject"
      ...
      # TODO : progressively fallback unless explicitly specified ?
      primitive_values=''
      u_asc_primitive_values 'variants' "$p_path/$subject"
...
#   primitive_values=''
#   u_asc_primitive_values 'actions' 'path/to/preset/folder'
