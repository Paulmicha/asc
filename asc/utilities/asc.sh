...
# - ASC_INC         # Files containing bash functions.
# - ASC_SUBJECTS    # TODO provide examples e.g. keys = 'app env git provision remote stack service task worker logger asc'
# - ASC_PREDICATES  # TODO provide examples
# - ASC_ACTIONS     # TODO provide examples e.g. keys = 'bootstrap init load reload unload install reinstall uninstall build rebuild start restart stop add remove process trigger watch compile test plan delay deploy destroy'
# - ASC_VARIANTS    # TODO provide examples e.g. keys = 'pre post'
#
# TODO remove this paragraph and explain details in doc instead. Proposition :
# A ASC "object" is a group formed by a single subject with its actions,
# predicates, and variants. Every folder of depth = 1 is a subject.
#
# This depth = 1 can apply to the ./asc folder itself ("core"), but that
...
u_asc_extend() {
  ...
  subjects_list="$(u_asc_get 'subjects' 'words' "$p_path" "$p_namespace")"
  ...
  for subject in $subjects_list; do
    ...
    # asc/bootstrap.sh scope directly (as it contains bash functions).
    ...
    # the corresponding ASC globals.
    for ext in $extensions; do
      # These are similar to .gitignore files : they whitelist ASC extensions.
...
#   # if [[ "$ext" == 'actions' ]]; then
#   #   eval "$(u_string_kss_write ASC_VARIANTS "pre_$added_val" "$p_path/$dir")"
#   #   eval "$(u_string_kss_write ASC_VARIANTS "post_$added_val" "$p_path/$dir")"
#   # fi
# }

##
# [internal] Basic ASC getter function (maps functions by given value name).
#
...
#
# @example
#   extensions="$(u_asc_get extensions words 'path/to/relative/dir')"
#   echo "$extensions"
#   # @see u_asc_get_extending_words()
#
u_asc_get() {
  ...
  case "$p_type" in
    ...
    # @see u_asc_get_extending_words()
    words)
      echo "$(u_asc_get_extending_words $p_which_values $@)"
    ;;

    # Default : attempt to call the function name "u_asc_$p_which_values".
    *)
      echo "$(u_asc_$p_which_values $@)"
    ;;
  esac
}
...
# variations in words for adding different ways to extend / alter ASC.
#
# @param 1 [optional] String relative path (defaults to './asc' = ASC "core").
# @param 2 [optional] String globals "namespace" (defaults to 'ASC').
#
...
#   words="$(u_asc_get_extending_words subjects 'path/to/relative/dir' 'MY_NAMESPACE')"
#   echo "$words"
#   # Yields for ex. "app db env git provision remote stack test instance"
#
#   # Example with extensions.
#   words="$(u_asc_get_extending_words extensions 'path/to/relative/dir' 'MY_NAMESPACE')"
#   echo "$words"
#   # Yields for ex. "predicates actions variants combos"
#
u_asc_get_extending_words() {
  local p_suffix="$1"
  local p_path="$2"
  local p_namespace="$3"
  ...
  local ext
  local words_list_file="$p_path/.asc_${p_suffix}"
  local file_contents
  ...
  # Finally, output the normal "$p_path/.asc_${p_suffix}" file contents (if it
  # exists).
