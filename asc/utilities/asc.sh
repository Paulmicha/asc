...
# @export ASC_PREFIXES (See 2.2)
# @export ASC_VARIANTS (See 2.3)
# @export ASC_PRESETS (See 3)
#
...
#     The dotfiles '.asc_actions' and '.asc_actions_append' have the same role
...
u_asc_extend() {
  ...
  local subjects_list
  subjects_list="$(u_asc_primitive_values 'subjects' "$p_path")"
  ...
  for subject in $subjects_list; do
    ...
    actions_list="$(u_asc_primitive_values 'actions' "$p_path/$subject")"
  ...
  # primitives.
  # Restrict this to ASC namespace only.
...
#
# @requires ASC_PRESETS global in calling scope.
# @see u_asc_extend()
#
u_asc_presets() {
  local presets_dir="asc/custom"
  if [[ -n "$ASC_CUSTOM_DIR" ]]; then
    presets_dir="$ASC_CUSTOM_DIR"
  fi
...
#
# @example
#   subjects="$(u_asc_primitive_values subjects)"
...
#   subjects="$(u_asc_primitive_values subjects 'path/to/relative/dir')"
#   echo "$subjects"
#
u_asc_primitive_values() {
  ...
  # Look for the dotfile that will override all default values.
  dotfile="$p_path/.asc_${p_primitive}"
  if [[ -f "$dotfile" ]]; then
  ...
  # Look for the dotfile that provides additional values.
  dotfile="$p_path/.asc_${p_primitive}_append"
  if [[ -f "$dotfile" ]]; then
    dotfile_contents=$(< "$dotfile")
    if [[ -n "$dotfile_contents" ]]; then
      local added_val
  ...
  dotfile="$p_path/.asc_${p_primitive}_ignore"

  # primitive_values="$(u_asc_primitive_values "$p_primitive" "$p_path")"
  # if [[ -z "$primitive_values" ]]; then
  #   primitive_values=$(u_fs_dir_list "$p_path")
  # fi

  # ignore="$(u_asc_primitive_values "${p_primitive}_ignore" "$p_path")"
  # if [[ -z "$ignore" ]]; then
  #   # TODO
  # fi
  ...
  # local dotfile="$p_path/.asc_${p_primitive}"
  # local file_contents
  ...
  # # Finally, output the normal "$p_path/.asc_${p_primitive}" file contents (if it
  # # exists).
  # if [[ -f "$dotfile" ]]; then
  #   cat "$dotfile"
  # fi
