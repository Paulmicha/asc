...
u_asc_extend() {
  ...
  subjects_list="$(u_asc_read 'subjects' "$p_path")"
  ...
  for subject in $subjects_list; do
    ...
    actions_list="$(u_asc_read 'actions' "$p_path/$subject")"
...
#
# @param 1 String dotfile name suffix - e.g. to produce ".asc_${p_suffix}".
...
#
# @example
#   subjects="$(u_asc_read subjects 'path/to/relative/dir')"
#   echo "$subjects"
...
u_asc_read() {
  ...
  local ext
  local dotfile="$p_path/.asc_${p_suffix}"
  local file_contents
