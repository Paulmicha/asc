...
u_env_models_get_lookup_paths() {
  ...
  if [[ -n "${p_arr[1]}" ]]; then
    ...
    u_array_add_once "asc/provision/${PROVISION_USING}/vars.sh" ENV_MODELS_PATHS
  fi
  ...
  for stack_service in "${STACK_SERVICES[@]}"; do
    ...
    if [[ -n "${ss_arr[1]}" ]]; then
      ...
      u_array_add_once "asc/provision/${stack_service}/vars.sh" ENV_MODELS_PATHS
      u_autoload_add_lookup_level "asc/provision/${stack_service}/" 'vars.sh' "$PROVISION_USING" ENV_MODELS_PATHS
    fi
  done
  ...
  for stack_preset in "${STACK_PRESETS[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      ...
      u_array_add_once "asc/provision/presets/$APP/${stack_preset}/vars.sh" ENV_MODELS_PATHS
      u_autoload_add_lookup_level "asc/provision/presets/$APP/${stack_preset}/" 'vars.sh' "$PROVISION_USING" ENV_MODELS_PATHS
  ...
  # App-related models.
  u_array_add_once "asc/app/${APP}/env.vars.sh" ENV_MODELS_PATHS
  u_autoload_add_lookup_level "asc/app/${APP}/env." 'vars.sh' "$PROVISION_USING" ENV_MODELS_PATHS
