  ...
  for stack_preset in "${STACK_PRESETS[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      for sp_type in $sp_types; do
        u_env_models_lookup_version "asc/$sp_type/presets/${sp_arr[0]}" "${sp_arr[1]}" true
      done
    else
      for sp_type in $sp_types; do
        u_array_add_once "asc/$sp_type/presets/${stack_preset}/vars.sh" ENV_MODELS_PATHS
        u_autoload_add_lookup_level "asc/$sp_type/presets/${stack_preset}/" 'vars.sh' "$PROVISION_USING" ENV_MODELS_PATHS
      done
    fi
  done
