...
#
# Unless "-y" argument was used in asc/stack/init.sh call, this will also prompt
...
u_env_models_get_lookup_paths() {
  ...
  ENV_MODELS_PATHS=()
  u_env_models_lookup_ca_provisioning "asc/provision/"
  ...
  for stack_service in "${STACK_SERVICES[@]}"; do
    ...
    if [[ -n "${ss_arr[1]}" ]]; then
      u_env_models_lookup_ca_provisioning "asc/provision/${ss_arr[0]}/"
      u_env_models_lookup_ca_provisioning "asc/provision/${ss_arr[0]}/${ss_arr[1]}/"
    else
      u_env_models_lookup_ca_provisioning "asc/provision/${stack_service}/"
    fi
  done
  ...
  for stack_preset in "${STACK_PRESETS[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/${sp_arr[0]}/"
      u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/${sp_arr[0]}/${sp_arr[1]}/"
      if [[ -n "$APP_VERSION" ]]; then
        u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/$APP_VERSION/${sp_arr[0]}/"
        u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/$APP_VERSION/${sp_arr[0]}/${sp_arr[1]}/"
      fi
    else
      u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/${stack_preset}/"
      if [[ -n "$APP_VERSION" ]]; then
        u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/$APP_VERSION/${stack_preset}/"
      fi
    fi
  done

  ENV_MODELS_PATHS+=("asc/app/${APP}/env.vars.sh")
  if [[ -n "$APP_VERSION" ]]; then
    ENV_MODELS_PATHS+=("asc/app/${APP}/$APP_VERSION/env.vars.sh")
    u_env_models_lookup_ca_provisioning "asc/app/${APP}/$APP_VERSION/env."
  fi
}
...
u_stack_get_specs() {
  ...
  # following files :
  # - asc/app/$APP/required_services.sh :
  #     lists services required by any $APP version.
  # - asc/app/$APP/$APP_VERSION/required_services.sh :
  #     lists, complements or overrides services required by that specific version.
  if [[ -f "asc/app/$APP/required_services.sh" ]]; then
    . "asc/app/$APP/required_services.sh"
    if [[ -f "asc/app/$APP/$APP_VERSION/required_services.sh" ]]; then
      . "asc/app/$APP/$APP_VERSION/required_services.sh"
    fi
  ...
  if [[ -n "$variants" ]]; then
    ...
    # the first one.
    # Example : asc/app/drupal/required_services.sh
    local key
    local option
