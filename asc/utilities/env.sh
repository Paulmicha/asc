...
u_env_models_get_lookup_paths() {
  ...
  for stack_service in "${STACK_SERVICES[@]}"; do
    ...
    if [[ -n "${ss_arr[1]}" ]]; then
      u_env_models_lookup_version "asc/provision/${ss_arr[0]}" "${ss_arr[1]}" true
    else
      u_env_models_path_add_once "asc/provision/${stack_service}/vars.sh"
  ...
  for stack_preset in "${STACK_PRESETS[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      # u_env_models_path_add_once "asc/provision/presets/$APP/${sp_arr[0]}/vars.sh"
      # u_env_models_lookup_ca_provisioning "asc/provision/presets/$APP/${sp_arr[0]}/"
      u_env_models_lookup_version "asc/provision/presets/$APP/${sp_arr[0]}" "${sp_arr[1]}" true

      if [[ -n "$APP_VERSION" ]]; then
        app_path="asc/provision/presets/$APP"
      ...
      u_env_models_path_add_once "asc/provision/presets/$APP/${stack_preset}/vars.sh"
  ...
  # App-related models.
  u_env_models_path_add_once "asc/app/${APP}/env.vars.sh"
  u_env_models_lookup_ca_provisioning "asc/app/${APP}/env."

  if [[ -n "$APP_VERSION" ]]; then
    app_path="asc/app/$APP"
    for app_v in "${app_version_arr[@]}"; do
      app_path+="/$app_v"
