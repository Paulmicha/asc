...
# @see asc/stack/init/aggregate_deps.sh
#
# @example
#   u_stack_resolve_deps
#   for stack_preset in "${STACK_PRESETS[@]}"; do
...
u_stack_deps_get_lookup_paths() {
  ...
  # u_autoload_add_lookup_level "asc/provision/" 'dependencies.sh' "$PROVISION_USING" DEPS_LOOKUP_PATHS
  DEPS_LOOKUP_PATHS+=("asc/provision/${HOST_TYPE}_host.dependencies.sh")
  u_autoload_add_lookup_level "asc/provision/" 'dependencies.sh' "$HOST_OS" DEPS_LOOKUP_PATHS
  u_autoload_add_lookup_level "asc/provision/" "${HOST_TYPE}_host.dependencies.sh" "$HOST_OS" DEPS_LOOKUP_PATHS
  u_autoload_add_lookup_level "asc/provision/" 'dependencies.sh' "$PROVISION_USING" DEPS_LOOKUP_PATHS "$HOST_OS"
  u_autoload_add_lookup_level "asc/provision/" "${HOST_TYPE}_host.dependencies.sh" "$PROVISION_USING" DEPS_LOOKUP_PATHS "$HOST_OS"

  # App-related dependencies.
  DEPS_LOOKUP_PATHS+=("asc/app/$APP/dependencies.sh")
  # u_autoload_add_lookup_level "asc/app/$APP/" 'dependencies.sh' "$PROVISION_USING" DEPS_LOOKUP_PATHS
  DEPS_LOOKUP_PATHS+=("asc/app/$APP/${HOST_TYPE}_host.dependencies.sh")

  u_autoload_add_lookup_level "asc/app/$APP/" 'dependencies.sh' "$HOST_OS" DEPS_LOOKUP_PATHS
  u_autoload_add_lookup_level "asc/app/$APP/" "${HOST_TYPE}_host.dependencies.sh" "$HOST_OS" DEPS_LOOKUP_PATHS

  u_autoload_add_lookup_level "asc/app/$APP/" 'dependencies.sh' "$PROVISION_USING" DEPS_LOOKUP_PATHS "$HOST_OS"
  u_autoload_add_lookup_level "asc/app/$APP/" "${HOST_TYPE}_host.dependencies.sh" "$PROVISION_USING" DEPS_LOOKUP_PATHS "$HOST_OS"

  u_autoload_add_lookup_level "asc/app/$APP/" 'dependencies.sh' "$INSTANCE_TYPE" DEPS_LOOKUP_PATHS "$HOST_OS"
  u_autoload_add_lookup_level "asc/app/$APP/" "${HOST_TYPE}_host.dependencies.sh" "$INSTANCE_TYPE" DEPS_LOOKUP_PATHS "$HOST_OS"
  u_autoload_add_lookup_level "asc/app/$APP/" 'dependencies.sh' "$INSTANCE_TYPE" DEPS_LOOKUP_PATHS "$PROVISION_USING"
  u_autoload_add_lookup_level "asc/app/$APP/" "${HOST_TYPE}_host.dependencies.sh" "$INSTANCE_TYPE" DEPS_LOOKUP_PATHS "$PROVISION_USING"

  if [[ -n "$APP_VERSION" ]]; then
    local v
    local path="asc/app/$APP"
    local version_arr=()
