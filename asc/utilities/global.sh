...
#
# Result (git-ignored) : asc/env/current/global.vars.sh
#
# @see u_instance_init()
#
u_global_write() {
  ...
  echo "Writing global (env) vars to asc/env/current/global.vars.sh ..."

  # (Re)init destination file (make empty).
  cat > asc/env/current/global.vars.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
  ...
  for global_name in ${GLOBALS['.sorting']}; do
    ...
    eval "[[ -z \"\$$global_name\" ]] && echo \"readonly $global_name\"=\'\' >> asc/env/current/global.vars.sh"
    eval "[[ -n \"\$$global_name\" ]] && echo \"readonly $global_name=\\\"\$$global_name\\\"\" >> asc/env/current/global.vars.sh"
  done

  echo "Writing global (env) vars to asc/env/current/global.vars.sh : done."
  echo
}
  ...
  if [ -n "$ASC_EXTENSIONS" ]; then
    ...
    for extension in $ASC_EXTENSIONS; do
      if [ -f "asc/extensions/$extension/global.vars.sh" ]; then
        global_lookup_paths+="asc/extensions/$extension/global.vars.sh "
      fi
    done
  fi
