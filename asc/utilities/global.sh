...
u_global_write() {
  ...
  echo "Writing global (env) vars to scripts/asc/local/global.vars.sh ..."

  # (Re)init destination files (make empty).
  echo -n '' > .env
  cat > scripts/asc/local/global.vars.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
  ...
  for global_name in ${GLOBALS['.sorting']}; do
    ...
    eval "[[ -z \"\$$global_name\" ]] && echo \"readonly $global_name\"=\'\' >> scripts/asc/local/global.vars.sh"
    eval "[[ -n \"\$$global_name\" ]] && echo \"readonly $global_name=\\\"\$$global_name\\\"\" >> scripts/asc/local/global.vars.sh"
  ...
  echo "Writing global (env) vars to scripts/asc/local/global.vars.sh : done."
  echo
}
