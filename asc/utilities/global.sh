...
#   remote_domain='remote.instance.asc.com'
#   declarations+=("[append]=$remote_domain [to]=domains")
#   declarations+=("[append]=dev [to]=$remote_domain|type")
#   declarations+=("[append]='ssh -p123 username@asc.com' [to]=$remote_domain|connect")
...
u_global_update_var() {
  ...
  local settings='asc/env/current/vars.sh'
...
u_global_get_includes_lookup_paths() {
  ...
  if [[ -n "${p_arr[1]}" ]]; then
    local p_v
    local p_path="asc/provision/${p_arr[0]}"
    local p_version_arr=()
    ...
    u_array_add_once "asc/provision/${PROVISION_USING}/vars.sh" GLOBALS_INCLUDES_PATHS
  fi
  ...
  for stack_service in "${STACK_SERVICES[@]}"; do
    ...
    if [[ -n "${ss_arr[1]}" ]]; then
      u_global_get_includes_lookup_version "asc/provision/${ss_arr[0]}" "${ss_arr[1]}" true
    else
      u_array_add_once "asc/provision/${stack_service}/vars.sh" GLOBALS_INCLUDES_PATHS
      u_autoload_add_lookup_level "asc/provision/${stack_service}/" 'vars.sh' "$PROVISION_USING" GLOBALS_INCLUDES_PATHS
    fi
  done
  ...
  for stack_preset in "${STACK_PRESETS[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      for sp_type in $sp_types; do
        u_global_get_includes_lookup_version "asc/$sp_type/presets/${sp_arr[0]}" "${sp_arr[1]}" true
      done
    else
      for sp_type in $sp_types; do
        u_array_add_once "asc/$sp_type/presets/${stack_preset}/vars.sh" GLOBALS_INCLUDES_PATHS
        u_autoload_add_lookup_level "asc/$sp_type/presets/${stack_preset}/" 'vars.sh' "$PROVISION_USING" GLOBALS_INCLUDES_PATHS
      done
    fi
  done

  # App-related includes.
  u_array_add_once "asc/app/${APP}/env.vars.sh" GLOBALS_INCLUDES_PATHS
  u_autoload_add_lookup_level "asc/app/${APP}/env." 'vars.sh' "$PROVISION_USING" GLOBALS_INCLUDES_PATHS
