...
# @see scripts/asc/local/make_args_check.sh
#

# Use the complete generated check if it exists.
if [ -f scripts/asc/local/make_args_check.sh ]; then
  scripts/asc/local/make_args_check.sh $@
  exit $?
fi
  ...
  for entry in $reserved_values; do
    case "$1" in "$entry")
      ...
      case "$make_entry_point" in
        init)
          echo "asc/instance/init.make.sh $rest_of_args" >&2
          ;;
        init-debug)
          echo "asc/instance/init.make.sh -d -r $rest_of_args" >&2
          ;;
        setup)
          echo "asc/instance/setup.sh $rest_of_args" >&2
          ;;
        hook)
          echo "asc/instance/hook.make.sh $rest_of_args" >&2
          ;;
        hook-debug)
          echo "asc/instance/hook.make.sh -d -t $rest_of_args" >&2
          ;;
        globals-lp)
          echo "asc/env/global_lookup_paths.make.sh $rest_of_args" >&2
          ;;
        self-test)
          echo "asc/test/self_test.sh $rest_of_args" >&2
          ;;
      esac
