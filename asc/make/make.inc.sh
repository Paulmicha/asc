...
u_make_check_args() {
  ...
  mk_tasks+=('init')
  mk_entry_points+=('asc/instance/init.make.sh')
  mk_tasks+=('init-debug')
  mk_entry_points+=('asc/instance/init.make.sh -d -r')
  mk_tasks+=('setup')
  mk_entry_points+=('asc/instance/setup.sh')
  mk_tasks+=('hook')
  mk_entry_points+=('asc/instance/hook.make.sh')
  mk_tasks+=('hook-debug')
  mk_entry_points+=('asc/instance/hook.make.sh -d -t')
  mk_tasks+=('globals-lp')
  mk_entry_points+=('asc/env/global_lookup_paths.make.sh')
  mk_tasks+=('self-test')
  mk_entry_points+=('asc/test/self_test.sh')
...
##
# Writes make entrypoints to scripts/asc/local/default.mk
#
...
EOF
  ...
  for index in "${!mk_entry_points[@]}"; do
    ...
    echo ".PHONY: $task
$task:
	@ scripts/asc/local/make_args_check.sh \$(MAKECMDGOALS) && ${mk_entry_points[index]} \$(filter-out \$@,\$(MAKECMDGOALS))
" >> scripts/asc/local/default.mk
  ...
  echo "Writing arguments checker script scripts/asc/local/make_args_check.sh ..."

  cat > scripts/asc/local/make_args_check.sh <<'SHELL_SCRIPT_HEAD'
...
'
  ...
  echo "reserved_values=\"$reserved_values\"" >> scripts/asc/local/make_args_check.sh

  cat >> scripts/asc/local/make_args_check.sh <<SHELL_SCRIPT_BODY
...
SHELL_SCRIPT_BODY
  ...
  echo "Writing arguments checker script scripts/asc/local/make_args_check.sh : done."
  echo
}
